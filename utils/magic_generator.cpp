...
int rook_bits[64] = {12, 11, 11, 11, 11, 11, 11, 12, 11, 10, 10, 10, 10,
...
int bishop_bits[64] = {6, 5, 5, 5, 5, 5, 5, 6, 5, 5, 5, 5, 5, 5, 5, 5,
...
Bitboard random_bitboard() { return (Bitboard)rng(); }
...
// Tutaj musisz mieć też te funkcje "on_the_fly" (Ray-casting), które napisałeś
// wcześniej Służą one jako "wyrocznia" do sprawdzenia, czy magiczna liczba
// działa poprawnie.
...
Bitboard find_magic(int sq, int m_bits, bool is_bishop) {
  ...
  for (int k = 0; k < 100000000; k++) {
    ...
    for (int i = 0; i < num_indices; i++) {
      ...
      if (used[idx] == 0) {
        used[idx] =
            attacks[i]; // Pierwszy raz widzimy ten indeks - zapisujemy wynik
...
int main() {
  ...
  for (int i = 0; i < 64; i++) {
    cout << "  0x" << hex << find_magic(i, bishop_bits[i], true) << "ULL,"
         << endl;
  }
  cout << "};" << endl << endl;
  ...
  for (int i = 0; i < 64; i++) {
    cout << "  0x" << hex << find_magic(i, rook_bits[i], false) << "ULL,"
         << endl;
  }
  cout << "};" << endl;
