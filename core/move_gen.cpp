...
uint64_t Board::getPawnSinglePushes(Color color) const {
  uint64_t pawns =
      pieces[static_cast<int>(color)][static_cast<int>(PieceType::PAWN)];
...
uint64_t Board::getPawnDoublePushes(Color color) const {
  ...
  if (color == Color::WHITE) {
    // Białe piony mogą wykonać podwójny ruch, jeśli są na 2. linii (bit
    // przesunięty o 16 w lewo)
    return ((singlePushes << 8) & emptySquares & RANK_4_MASK);
  } else {
    // Czarne piony mogą wykonać podwójny ruch, jeśli są na 7. linii (bit
    // przesunięty o 16 w prawo)
...
uint64_t Board::getPawnAttacks(Color color) const {
  ...
  uint64_t opponentPieces =
      getColorOccupied(color == Color::WHITE ? Color::BLACK : Color::WHITE);
...
bool Board::isSquareAttacked(int sq, Color side) const {
  ...
  // 1. Skoczki
  if (getKnightAttacks(1ULL << sq) &
      pieces[static_cast<int>(side)][static_cast<int>(PieceType::KNIGHT)])
    return true;

  // 2. Król
  if (getKingAttacks(1ULL << sq) &
      pieces[static_cast<int>(side)][static_cast<int>(PieceType::KING)])
    return true;

  // 3. Wieże i Hetmany
  if (getTowerAttacks(sq) &
      (pieces[static_cast<int>(side)][static_cast<int>(PieceType::ROOK)] |
       pieces[static_cast<int>(side)][static_cast<int>(PieceType::QUEEN)]))
    return true;

  // 4. Gońce i Hetmany
  if (getBishopAttacks(sq) &
      (pieces[static_cast<int>(side)][static_cast<int>(PieceType::BISHOP)] |
       pieces[static_cast<int>(side)][static_cast<int>(PieceType::QUEEN)]))
    return true;

  // 5. Pionki
  if (getPawnAttacks(oppositeColor(side), 1ULL << sq) &
      pieces[static_cast<int>(side)][static_cast<int>(PieceType::PAWN)])
    return true;
...
PieceType Board::getPieceAt(int sq, Color color) const {
  int8_t entry = mailbox[sq];
  int colorIdx = static_cast<int>(color);
  if (entry == -1 || (entry >> 3) != colorIdx)
    return PieceType::NONE;
  return static_cast<PieceType>(entry & 7);
}

std::vector<Move> Board::generatePseudoLegalMoves(Color side) const {
  std::vector<Move> moves;
  moves.reserve(80); // Typowa max liczba ruchów ~80, unikamy realokacji

  int sideIdx = static_cast<int>(side);
  Color oppColor = (side == Color::WHITE) ? Color::BLACK : Color::WHITE;
  int oppIdx = static_cast<int>(oppColor);

  uint64_t myPieces = getColorOccupied(side);
  uint64_t opponentPieces = getColorOccupied(oppColor);
  uint64_t allPieces =
      myPieces | opponentPieces; // jedno wywołanie, a nie wielokrotne
  ...
  for (PieceType type : sliders) {
    ...
    while (sliderBB) {
      ...
      if (type == PieceType::ROOK)
        attacks = getTowerAttacks(fromSq);
      else if (type == PieceType::BISHOP)
        attacks = getBishopAttacks(fromSq);
      else
        attacks = getQueenAttacks(fromSq);
  ...
  if (king) {
    ...
    // --- ROSZADY ---
    // Warunki: Prawa do roszady, brak figur pomiędzy, król nie jest w szachu
    // (sprawdzane w LegalMoves)
    if (side == Color::WHITE) {
      // Krótka (WK) - bit 0 (wartość 1)
      if ((castlingRights & 1) && !((1ULL << 5) & allPieces) &&
          !((1ULL << 6) & allPieces)) {
        // Sprawdzamy czy pola f1, g1 nie są atakowane
        if (!isSquareAttacked(4, Color::BLACK) &&
            !isSquareAttacked(5, Color::BLACK) &&
            !isSquareAttacked(6, Color::BLACK))
          moves.emplace_back(4, 6, PieceType::KING, PieceType::NONE,
                             PieceType::NONE, CASTLING);
      }
      // Długa (WQ) - bit 1 (wartość 2)
      if ((castlingRights & 2) && !((1ULL << 1) & allPieces) &&
          !((1ULL << 2) & allPieces) && !((1ULL << 3) & allPieces)) {
        if (!isSquareAttacked(4, Color::BLACK) &&
            !isSquareAttacked(3, Color::BLACK) &&
            !isSquareAttacked(2, Color::BLACK))
          moves.emplace_back(4, 2, PieceType::KING, PieceType::NONE,
                             PieceType::NONE, CASTLING);
      }
    } else {
      // Czarna Krótka (BK) - bit 2 (wartość 4)
      if ((castlingRights & 4) && !((1ULL << 61) & allPieces) &&
          !((1ULL << 62) & allPieces)) {
        if (!isSquareAttacked(60, Color::WHITE) &&
            !isSquareAttacked(61, Color::WHITE) &&
            !isSquareAttacked(62, Color::WHITE))
          moves.emplace_back(60, 62, PieceType::KING, PieceType::NONE,
                             PieceType::NONE, CASTLING);
      }
      // Czarna Długa (BQ) - bit 3 (wartość 8)
      if ((castlingRights & 8) && !((1ULL << 57) & allPieces) &&
          !((1ULL << 58) & allPieces) && !((1ULL << 59) & allPieces)) {
        if (!isSquareAttacked(60, Color::WHITE) &&
            !isSquareAttacked(59, Color::WHITE) &&
            !isSquareAttacked(58, Color::WHITE))
          moves.emplace_back(60, 58, PieceType::KING, PieceType::NONE,
                             PieceType::NONE, CASTLING);
      }
    }
  }
  ...
  while (pawns) {
    ...
    if (!((1ULL << toSq) & allPieces)) {
      if (toSq / 8 == promotionRank) {
        moves.emplace_back(fromSq, toSq, PieceType::PAWN, PieceType::NONE,
                           PieceType::QUEEN, PROMOTION);
        moves.emplace_back(fromSq, toSq, PieceType::PAWN, PieceType::NONE,
                           PieceType::ROOK, PROMOTION);
        moves.emplace_back(fromSq, toSq, PieceType::PAWN, PieceType::NONE,
                           PieceType::BISHOP, PROMOTION);
        moves.emplace_back(fromSq, toSq, PieceType::PAWN, PieceType::NONE,
                           PieceType::KNIGHT, PROMOTION);
      } else {
        moves.emplace_back(fromSq, toSq, PieceType::PAWN, PieceType::NONE);
        // 2. Ruch o dwa pola
        int doublePushSq = fromSq + 2 * direction;
        if (fromSq / 8 == startRank && !((1ULL << doublePushSq) & allPieces)) {
          moves.emplace_back(fromSq, doublePushSq, PieceType::PAWN,
                             PieceType::NONE);
        }
      }
    }
    ...
    if (epCapture) {
      ...
      moves.emplace_back(fromSq, epSq, PieceType::PAWN, PieceType::PAWN,
                         PieceType::NONE, EN_PASSANT);
    }
...
std::vector<Move> Board::generateLegalMoves(Color side) {
  ...
  for (const auto &m : pseudoMoves) {
    ...
    int kingSq = __builtin_ctzll(
        pieces[static_cast<int>(side)][static_cast<int>(PieceType::KING)]);

    // Jeśli pole króla nie jest atakowane przez przeciwnika - ruch jest
    // legalny!
